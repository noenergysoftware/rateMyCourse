...
//confirmclick 提交追加评论的？ 根据submitDiscuss 暂不需要
...
function generateGrid(imageUrls, userName, iTerm, iTeacher, iTotal, text, time, commentid, cnum, snum) {
    var ScreenGridHtml = `
        ...
        commentGrid.setAttribute("class", "container");
      ...
      //  imageTag[0].setAttribute("class", "col-md-2");
        ...
        */
        ...
        pTags[0].setAttribute("class", "userName col-md-1");
       ...
       /* pTags[1].appendChild(term);
        ...
        pTags[7].setAttribute("style", "margin-top:16px;margin-left:16px;text-align:left; width:70%")
        //inset time
        ...
        /*
        //追加评论 暂不需要
        //ccomment 大概就是评论的评论，即追加评论 cnum为追加评论的数目
        ...
        */
        ...
        aTags[1].setAttribute("style", "float:right;text-align:right;margin-top:16px;margin-right:16px;");
 ...
 function gclick(node){
  ...
  $.ajax('/changeSupport/', {
        ...
        if(data.statCode >= 0){
            ...
            //r最终取到的是点赞的数量 在括号里面
            //应该是用于点赞或者取消赞 根据返回的statCode值来判断是否之前已经点赞了
            //为0表示没有点赞，为1表示已经点赞了
            if(data.statCode == 0){
